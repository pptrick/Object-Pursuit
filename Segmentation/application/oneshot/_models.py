...
def select_model(model,
    ...
    if model == "unet":
        ...
        net = Singlenet(z_dim=z_dim, device=device)
        net.init_hypernet(pretrained_hypernet, freeze=True)
        net.init_backbone(pretrained_backbone, freeze=True)
    elif model == "coeffnet": # combine bases and pursuit coeffs
        assert base_dir is not None
        net = Coeffnet(base_dir=base_dir, z_dim=z_dim, device=device, hypernet_path=pretrained_hypernet, backbone_path=pretrained_backbone)
